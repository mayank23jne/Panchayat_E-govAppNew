...
//           <Text className="text-blue-400 text-[10px]">▼</Text>
//         </TouchableOpacity>
...
//               <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">कुल मद</Text>
...
//             <View className="flex-1 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
//               <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">कुल फंड की संख्या</Text>
...
//             <View className="flex-1 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
//               <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">कुल आवंटित ग्राम पंचायत</Text>
...
//             {/* CARD 4: TOTAL GRAM */}
//             <View className="flex-1 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
//               <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">कुल आवंटित ग्राम</Text>
//               <Text className="text-2xl font-black text-slate-800" numberOfLines={1}>
    ...
    <SafeAreaView edges={["left", "right"]} className="flex-1 bg-slate-50">
      ...
      {/* DUAL DROPDOWN FILTER MANAGEMENT STRIP */}
        <View className="bg-white px-4 pt-4 pb-2">
                      <Text className="text-1xl font-black text-slate-500 tracking-tight">डैशबोर्ड</Text>
                    </View>
      <View className="bg-white p-4 border-b border-slate-200 z-50 shadow-sm flex-row gap-x-2">
        {/* Left Filter Column: Year Select */}
        <View className="flex-1 relative">
          <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">आवंटन वर्ष</Text>
          <TouchableOpacity 
            onPress={() => {
          ...
          >
            <Text className="text-blue-700 font-bold text-xs">{selectedYear}</Text>
            <Text className="text-blue-500 text-[9px]">▼</Text>
          </TouchableOpacity>

          {showYearDropdown && (
            <View className="bg-white border border-slate-200 rounded-xl shadow-xl absolute left-0 top-14 w-full z-50 overflow-hidden">
              <ScrollView className="max-h-48">
                {yearsList.map((yr) => (
                  <TouchableOpacity
                    key={yr}
                  ...
                  >
                    <Text className={`text-xs ${selectedYear === yr ? "font-bold text-blue-600" : "text-slate-700"}`}>{yr}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            </View>
        ...
        <View className="flex-1.5 relative">
          <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">कार्य स्थिति फ़िल्टर</Text>
          <TouchableOpacity 
            onPress={() => {
              setShowStatusDropdown(!showStatusDropdown);
              setShowYearDropdown(false);
            }}
            className="bg-purple-50/70 border border-purple-100 px-3 py-2 rounded-xl flex-row justify-between items-center active:bg-purple-100"
          >
            <Text className="text-purple-700 font-bold text-xs" numberOfLines={1}>
              {statusCategories.find(c => c.value === selectedCategory)?.label}
            </Text>
            <Text className="text-purple-500 text-[9px]">▼</Text>
          </TouchableOpacity>

          {showStatusDropdown && (
            <View className="bg-white border border-slate-200 rounded-xl shadow-xl absolute right-0 top-14 w-full z-50 overflow-hidden">
              {statusCategories.map((cat) => (
                ...
                >
                  <Text className={`text-xs ${selectedCategory === cat.value ? "font-bold text-purple-600" : "text-slate-700"}`}>
                    {cat.label}
                  </Text>
                </TouchableOpacity>
              ))}
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        <View className="p-4 gap-y-3">
          ...
          <View className="flex-row gap-x-3">
            <View className="flex-1 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
              <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">कुल ग्राम पंचायत</Text>
              <Text className="text-2xl font-black text-slate-800">
                {(dashboardData.metrics?.totalGramPanchayats || 0).toLocaleString('en-IN')}
              </Text>
            </View>

            <View className="flex-1 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
              <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">लंबित मद की संख्या</Text>
              <Text className="text-2xl font-black text-slate-800">
          ...
          <View className="flex-row gap-x-3">
            <View className="flex-1 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
              <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">कुल मद</Text>
              <Text className="text-lg font-black text-blue-600" numberOfLines={1}>
                ₹{(dashboardData.metrics?.totalAmount || 0).toLocaleString('en-IN')}
              </Text>
            </View>
